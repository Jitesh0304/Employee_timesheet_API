...
from account.serializers import CustomTokenObtainPairSerializer, CustomTokenRefreshSerializer, \
            ...
            SendPasswordResetEmailSerializer, EmployeePasswordResetSerializer, EmployeeRegistrationByTeamLeaderSerializer
...
# from .custom_auth import CustomEmployeeAuthentication
...
class EmployeeRegistrationByTeamLeaderView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, format =None):
        try:
            ...
            if  user.is_manager:
                serializer = EmployeeRegistrationByTeamLeaderSerializer(data = request.data, context={'request':request})
                if serializer.is_valid():
                        ## create new user
