    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            for week, days in weeks_till_today.items():
                ...
                if week_data_for_week.exists():
                    serializer = TimesheetRetrieveSerializer(week_data_for_week, many=True)
                    if week_data_for_week.filter(submit=True, approve=True):
                        all_data['Approve'][week] = serializer.data
                    elif week_data_for_week.filter(submit=True, approve=False):
                        all_data['Submit'][week] = serializer.data
                    elif week_data_for_week.filter(submit=False, approve=False):
    ...
    def post(self, request,format=None):
        ...
        try:
            if user.is_manager:
                ...
                timesheet_data = Timesheet.objects.filter(project_name__projectManager=user, submit=True, employee=employee_id,
                                                          approve=False, date__range=[week_start_date, week_end_date])
                ...
                if req_approve=="True":
                    ...
                    onereport = WeeklyReport.objects.filter(submit_timesheet__submit=True, employee__empID=employee_id,
                                                            ...
                                                            submit_timesheet__approve=False).distinct()
                    onereport.update(approve=True)
                    total = timesheet_data.update(approve=True)
                    message = f"Total {total} timesheet data approved..."
                else:
                    total = timesheet_data.update(approve=False, submit=False)
